...
def resolve_animation(request, pixels):
	...
	tail_length = parse_int(form[tail_length]) if 'tail_length' in form else 0
	...
	step = parse_int(form['step']) if 'step' in form else 0
	num_sparkles = parse_int(form['num_sparkles']) if 'num_sparkles' in form else 0
